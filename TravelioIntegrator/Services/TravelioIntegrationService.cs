...
public class TravelioIntegrationService
{
    ...
    public async Task<bool?> VerificarDisponibilidadItemAsync(TipoServicio tipo, int itemId, ILogger? logger = null)
    {
        ...
        try
        {
            ...
            {
                case TipoServicio.Aerolinea:
                    var vuelo = await _db.CarritosAerolinea.Include(c => c.Servicio).ThenInclude(s => s.DetalleServicio)
                        .FirstOrDefaultAsync(c => c.Id == itemId);
                    if (vuelo?.Servicio.DetalleServicio is null) return null;
                    {
                        var uri = BuildUri(vuelo.Servicio.DetalleServicio, vuelo.Servicio.DetalleServicio.ConfirmarProductoEndpoint);
                ...
                case TipoServicio.Hotel:
                    var hab = await _db.CarritosHabitaciones.Include(c => c.Servicio).ThenInclude(s => s.DetalleServicio)
                        .FirstOrDefaultAsync(c => c.Id == itemId);
                    if (hab?.Servicio.DetalleServicio is null) return null;
                    {
                        var uri = BuildUri(hab.Servicio.DetalleServicio, hab.Servicio.DetalleServicio.ConfirmarProductoEndpoint);
                ...
                case TipoServicio.RentaVehiculos:
                    var auto = await _db.CarritosAutos.Include(c => c.Servicio).ThenInclude(s => s.DetalleServicio)
                        .FirstOrDefaultAsync(c => c.Id == itemId);
                    if (auto?.Servicio.DetalleServicio is null) return null;
                    {
                        var uri = BuildUri(auto.Servicio.DetalleServicio, auto.Servicio.DetalleServicio.ConfirmarProductoEndpoint);
                ...
                case TipoServicio.PaquetesTuristicos:
                    var paq = await _db.CarritosPaquetes.Include(c => c.Servicio).ThenInclude(s => s.DetalleServicio)
                        .FirstOrDefaultAsync(c => c.Id == itemId);
                    if (paq?.Servicio.DetalleServicio is null) return null;
                    {
                        var uri = BuildUri(paq.Servicio.DetalleServicio, paq.Servicio.DetalleServicio.ConfirmarProductoEndpoint);
                ...
                case TipoServicio.Restaurante:
                    var mesa = await _db.CarritosMesas.Include(c => c.Servicio).ThenInclude(s => s.DetalleServicio)
                        .FirstOrDefaultAsync(c => c.Id == itemId);
                    if (mesa?.Servicio.DetalleServicio is null) return null;
                    {
                        var uri = BuildUri(mesa.Servicio.DetalleServicio, mesa.Servicio.DetalleServicio.ConfirmarProductoEndpoint);
    ...
    public async Task<bool?> CrearHoldsParaUsuarioAsync(int clienteId, int duracionHoldSegundos = 300, ILogger? logger = null)
    {
        ...
        try
        {
            ...
            var vuelos = await _db.CarritosAerolinea.Include(c => c.Pasajeros)
                .Include(c => c.Servicio).ThenInclude(s => s.DetalleServicio)
                .Where(c => c.ClienteId == clienteId)
                .ToListAsync();
            foreach (var item in vuelos)
            {
                if (item.Servicio.DetalleServicio is null) continue;
                var det = item.Servicio.DetalleServicio;
            ...
            var habitaciones = await _db.CarritosHabitaciones.Include(c => c.Servicio).ThenInclude(s => s.DetalleServicio)
                .Where(c => c.ClienteId == clienteId)
                .ToListAsync();
            ...
            var autos = await _db.CarritosAutos.Include(c => c.Servicio).ThenInclude(s => s.DetalleServicio)
                .Where(c => c.ClienteId == clienteId)
                .ToListAsync();
            ...
            var mesas = await _db.CarritosMesas.Include(c => c.Servicio).ThenInclude(s => s.DetalleServicio)
                .Where(c => c.ClienteId == clienteId)
                .ToListAsync();
